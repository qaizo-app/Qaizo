# Qaizo — Google Play Store Listing

## App Name
Qaizo — Smart Finance Manager
...
- Receipt scanner:
Take a photo of any receipt or pick from gallery. The app reads the amount, store, date, category and individual items automatically. Long receipt? Add multiple photos — the app combines them. Duplicate protection prevents scanning the same receipt twice;

- Smart shopping list:
All items from scanned receipts are collected into a smart shopping list. See price history, cheapest store, and get alerts when frequent items haven't been bought in a while. Build a shopping list and share it via WhatsApp;

- Built-in calculator:
Calculate percentages, fees and discounts right in the transaction form. No need to switch apps;
...
סריקת קבלות:
צלם קבלה או בחר מהגלריה. האפליקציה מזהה אוטומטית סכום, חנות, תאריך, קטגוריה ופריטים. קבלה ארוכה? הוסף כמה תמונות. הגנה מפני כפילויות

רשימת קניות חכמה:
כל הפריטים מקבלות סרוקות נאספים לרשימת קניות חכמה. היסטוריית מחירים, החנות הכי זולה, התראה כשמוצר תכוף לא נקנה. בנה רשימה ושתף בוואטסאפ

מחשבון מובנה:
חשב אחוזים, עמלות והנחות ישירות בטופס הוספת תנועה
...
בקרוב: קלט חכם בטקסט ויועץ פיננסי AI.
...
- Сканер чеков:
Сфотографируйте чек или выберите из галереи. Приложение автоматически определит сумму, магазин, дату, категорию и товары. Длинный чек? Добавьте несколько фото. Защита от дублирования;

- Умный список покупок:
Все товары из отсканированных чеков собираются в умный список. История цен, самый дешёвый магазин, предупреждения когда частый товар давно не покупался. Создайте список и поделитесь через WhatsApp;

- Встроенный калькулятор:
Считайте проценты, комиссии и скидки прямо в форме транзакции;
...
Скоро: умный ввод текстом и финансовый советник на базе AI.
...
## Data Safety Answers
- App collects: Email address, name (optional), financial data (user-entered)
